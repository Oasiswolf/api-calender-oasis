...
app.config['SQLALCHEMY_DATABASE_URI'] = "postgresql://hkyoqvaprzulgv:0d082b5766123617ece8e3bb2758622ea7637d79dfd3fe5b7fc1cf3037a08b69@ec2-52-70-205-234.compute-1.amazonaws.com:5432/deo0sctera1199"
...
# Get method does not need to be added in the pull request
@app.route("/month/get")
def get_all_months():
...
def add_reminder():
    ...
    if existing_reminder_check is not None:
        return jsonify("Error date already has reminder that already exists")
...
def delete_reminder_by_id(id):
    reminder_to_delete = db.session.query(Reminder).filter(Reminder.id == id).first()
    db.session.delete(reminder_to_delete)
    db.session.commit()
    return jsonify(reminder_schema.dump(reminder_to_delete))
